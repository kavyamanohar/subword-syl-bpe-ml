...
def encode(orig, bpe_codes, bpe_codes_reverse, vocab, separator, version, cache, glossaries_regex=None, dropout=0):
    ...
    try:
        syllabified_orig = mlphon.split_to_syllables(orig)
    except ValueError as error_instance:
        syllabified_orig = ['?']
    
    if len(syllabified_orig) == 1:
        return tuple(syllabified_orig)

    if version == (0, 1):
        word = syllabified_orig + ['</w>']
    elif version == (0, 2): # more consistent handling of word-final segments
        word = syllabified_orig
        word[-1] = word[-1] + '</w>'
    else:
...
def check_vocab_and_split(orig, bpe_codes, vocab, separator):
    ...
    out = []
    syl=mlphon.split_to_syllables(orig[:-1])
