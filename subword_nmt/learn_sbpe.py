...
def learn_sbpe(infile, outfile, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, num_workers=1):
    ...
    vocab = get_vocabulary(infile, is_dict, num_workers) # Count of each word in the corpus in descending order
    # vocab = dict([(tuple(x[:-1])+(x[-1]+'</w>',) ,y) for (x,y) in vocab.items()])
    vocab_dict = {}
    for (x,y) in vocab.items():
        try:
            syl = mlphon.split_to_syllables(x)
        except ValueError as error_instance:
            syl = ['?']
        syl[-1] =  syl[-1]+'</w>'
        syl_tuple = tuple(syl)
        vocab_dict[syl_tuple] = y
    vocab = vocab_dict
...
if __name__ == '__main__':
    ...
    learn_sbpe(args.input, args.output, args.symbols, args.min_frequency, args.verbose, is_dict=args.dict_input, total_symbols=args.total_symbols, num_workers=args.num_workers)
